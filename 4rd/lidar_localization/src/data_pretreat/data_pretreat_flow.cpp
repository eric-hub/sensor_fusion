...
DataPretreatFlow::DataPretreatFlow(ros::NodeHandle &nh,
  ...
  gnss_pub_ptr_ = std::make_shared<OdometryPublisher>(
      nh, "/synced_gnss", "/map", "/velo_link", 100);

  // motion compensation for lidar measurement:
  distortion_adjust_ptr_ = std::make_shared<DistortionAdjust>();

  origin_path_ = WORK_SPACE_PATH + "/slam_data/map";
  if (!FileManager::CreateDirectory(origin_path_, "点云地图文件"))
    std::cout << "创建点云地图文件夹失败" << std::endl;
...
bool DataPretreatFlow::ReadData() {
  ...
  bool valid_velocity = VelocityData::SyncData(unsynced_velocity_,
                                               velocity_data_buff_, cloud_time);
  bool valid_gnss =
      GNSSData::SyncData(unsynced_gnss_, gnss_data_buff_, cloud_time);
...
bool DataPretreatFlow::ValidData() {
  ...
  double diff_velocity_time =
      current_cloud_data_.time - current_velocity_data_.time;
  double diff_gnss_time = current_cloud_data_.time - current_gnss_data_.time;
  if (diff_imu_time < -0.05 || diff_velocity_time < -0.05 ||
      diff_gnss_time < -0.05) {
    cloud_data_buff_.pop_front();
    return false;
  }
