...
KITTIFiltering::KITTIFiltering()
    : global_map_ptr_(new CloudData::CLOUD()), local_map_ptr_(new CloudData::CLOUD()), current_scan_ptr_(new CloudData::CLOUD()) {
    // load ROS config:
    InitWithConfig();
}
...
bool KITTIFiltering::InitFusion(const YAML::Node& config_node) {
    ...
    CONFIG.FUSION_STRATEGY_ID["cons_vel"] = KalmanFilter::MeasurementType::CONS_VEL;
