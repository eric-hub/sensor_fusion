...
ErrorStateKalmanFilter::ErrorStateKalmanFilter(const YAML::Node &node) {
    ...
    GConsVel_.block<3, 3>(5, 5) = Eigen::Matrix3d::Identity();
    CConsVel_ = MatrixCConsVel::Identity();

    RConsVel_.block<3, 3>(0, 0) = COV.MEASUREMENT.POSE.POSI * Eigen::Matrix3d::Identity();
    RConsVel_.block<2, 2>(3, 3) = COV.MEASUREMENT.VEL * Eigen::Matrix2d::Identity();
    RConsVel_.block<3, 3>(5, 5) = COV.MEASUREMENT.POSE.ORI * Eigen::Matrix3d::Identity();
    ...
    QConsVel_.block<kDimMeasurementConsVel, kDimState>(0, 0) = GConsVel_;
}
    ...
    switch (measurement_type) {
        case MeasurementType::POSE:
            CorrectErrorEstimationPose(
                ...
                Y, G, K);
            break;
        case MeasurementType::CONS_VEL:
            //
            // TODO: register new correction logic here:
            //
            CorrectErrorEstimationConsVel(
