...
bool FrontEnd::InitRegistration(
  ...
  if (registration_method == "NDT") {
    ...
    */
    ...
    LOG(ERROR) << "Point cloud registration method " << registration_method
               << " NOT FOUND!";
    return false;
  }
...
bool FrontEnd::InitFilter(std::string filter_user,
  ...
  if (filter_mothod == "voxel_filter") {
    ...
    LOG(ERROR) << "Point cloud filter method " << filter_mothod
               << " NOT FOUND!";
    return false;
  }
...
bool FrontEnd::SaveMap() {
  ...
  for (size_t i = 0; i < global_map_frames_.size(); ++i) {
    key_frame_path =
        data_path_ + "/key_frames/key_frame_" + std::to_string(i) + ".pcd";
