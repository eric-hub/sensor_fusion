...
bool SICPRegistration::SetInputTarget(
...
bool SICPRegistration::ScanMatch(const CloudData::CLOUD_PTR &input_source,
  ...
  Eigen::Matrix3Xd X(
      3, transformed_input_source->points.size()); // source, transformed
  ...
  for (int i = 0; i < input_target_->points.size(); i++) {
  ...
  transformation_ =
      SICP::point_to_point(X, Y).matrix().cast<float>(); // sparse ICP
